...
TokenIdTensor = torch.Tensor
EmbeddingMatrix = torch.Tensor
PadTokenId = int
EncodedEmbeddings = torch.Tensor

CodebookEmbeddingFn = Callable[
    [TokenIdTensor, EmbeddingMatrix, PadTokenId], EncodedEmbeddings
]
...
class BaseEncoder(nn.Module, ABC, Generic[EncoderConfigType]):
    ...
    def get_codebook_embedding_fn(self) -> CodebookEmbeddingFn:
