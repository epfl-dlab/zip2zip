...
impl CodebookManager {
    ...
    #[getter]
    fn codebook(&self) -> HashMap<usize, Vec<usize>> {
        self.codebook.clone()
    }
...
fn bounded_lzw(
    ...
    max_subtokens: usize,
    disabled_ids: &Set,
) -> (Vec<usize>, Vec<usize>, FxHashMap<Vec<usize>, usize>) {
    let mut compressed_ids: Vec<usize> = Vec::new();
    let mut extra_merges = FxHashMap::default();
    ...
    while i < ids.len() && compressed_ids.len() < max_out_seq_length - 1 {
        ...
        if disabled_ids.contains(&id) {
            if ids_to_merge.len() > 0 {
                compressed_ids.push(get_usize_from_codebook(&extra_merges, &ids_to_merge));
                ids_to_merge.clear();
            }
            compressed_ids.push(id);
            i += 1;
        ...
        let is_in_codebook = codebook_contains(&extra_merges, &ids_to_merge, initial_vocab_size);
        if !is_in_codebook {
            if next_compressed_id < initial_vocab_size + extra_vocab_size {
...
fn bounded_lzw_in_chunks(
    ...
    let mut lzw_chunks = Vec::new();
    let disabled_ids = disabled_ids_to_set(disabled_ids);

    while !remaining_ids.is_empty() {
        let (compressed_ids, new_remaining_ids, merges) = bounded_lzw(
            remaining_ids,
            initial_vocab_size,
            extra_vocab_size,
            chunk_length,
        ...
        lzw_chunks.push((compressed_ids, hashable_merges));
...
#[pyfunction]
#[pyo3(name = "bounded_lzw_in_chunks")]
fn py_bounded_lzw_in_chunks(
    ids: Vec<usize>,
    initial_vocab_size: usize,
    extra_vocab_size: usize,
    max_out_seq_length: usize,
    ...
    let lzw_chunks = bounded_lzw_in_chunks(
        ids,
        initial_vocab_size,
        extra_vocab_size,
        max_out_seq_length,
...
#[pyfunction]
#[pyo3(name = "batch_bounded_lzw_in_chunks")]
fn py_batch_bounded_lzw_in_chunks(
    ids: Vec<Vec<usize>>,
    initial_vocab_size: usize,
    extra_vocab_size: usize,
    max_out_seq_length: usize,
...
fn zip2zip_compression(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(py_bounded_lzw_in_chunks, m)?)?;
    m.add_function(wrap_pyfunction!(py_batch_bounded_lzw_in_chunks, m)?)?;
...
mod tests {
    ...
    fn test_encode() {
        ...
        let (old_compressed_ids, _, _) = bounded_lzw(
            ids.clone(),
            initial_vocab_size,
            extra_vocab_size,
            max_out_seq_length,
    ...
    fn test_codebook_manager() {
        ...
        let (compressed_ids, _, codebook_map) = bounded_lzw(
            ids,
            initial_vocab_size,
            max_codebook_size,
            max_out_seq_length,
            max_subtokens,
            &disabled_ids_to_set(None), // Some(disabled_ids.clone()),
        );
    ...
    fn test_bounded_lzw_in_chunks() {
        ...
        assert_eq!(lzw_chunks.len(), 1, "Should produce exactly one chunk");
        let (compressed_ids, _codebook_dict) = &lzw_chunks[0];
    ...
    fn test_lzw_chunk_compress() {
        ...
        for (i, (compressed_ids, _)) in lzw_chunks.iter().enumerate() {
            if i < lzw_chunks.len() - 1 {
                assert_eq!(
                    compressed_ids.len(),
                    out_seq_length,
    ...
    fn test_no_disabled_ids_in_the_codebook() {
        ...
        let lzw_chunks =
            bounded_lzw_in_chunks(ids, 32011, 256, 1024, 8, Some(disabled_ids.clone()));

        for codebook in lzw_chunks.iter().map(|(_, codebook)| codebook) {
